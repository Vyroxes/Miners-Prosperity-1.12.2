...
import net.vyroxes.minersprosperity.objects.tileentities.TileEntityCreativeSolarPanel;
...
public class MachineCreativeSolarPanel extends Machine {
    public MachineCreativeSolarPanel(String name) {
    ...
    public @NotNull Item getItemDropped(IBlockState state, Random rand, int fortune) {
        return Item.getItemFromBlock(BlockInit.CREATIVE_SOLAR_PANEL);
    }

    @Override
    public TileEntityMachine getTileEntity(World world, BlockPos pos) {
        return (TileEntityCreativeSolarPanel) world.getTileEntity(pos);
    }
