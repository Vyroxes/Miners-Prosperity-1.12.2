...
public class TileEntityCreativeSolarPanel extends TileEntityMachine
{
    public TileEntityCreativeSolarPanel()
    {
        super(new Builder()
                .setItemEnergy(1)
                .setEnergy(Integer.MAX_VALUE, 0, Integer.MAX_VALUE, 0)
    ...
    {
        return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("container.creative_solar_panel");
    }
        ...
        for (EnumFacing facing : EnumFacing.VALUES)
        {
            ...
            if (neighbor != null && neighbor.hasCapability(CapabilityEnergy.ENERGY, facing.getOpposite()))
            {
                ...
                if (neighborStorage != null)
                {
                    ...
                    neighborStorage.receiveEnergy(energyExtracted, false);
                }
            }
        }
    }
    ...
    public void chargeItem(ItemStack stack) {
        ...
        itemEnergy.receiveEnergy(energyToReceive, false);
    }
    ...
    public void update()
    {
        ...
        if (!world.isRemote)
        {
            ...
            if (!hasBlockAbove && isDay)
            {
                if (isThunderstorm)
                {
                    this.energyGeneration = (int) (Integer.MAX_VALUE * 0.5);
                    if (this.storage.getEnergyStored() != this.storage.getMaxEnergyStored()) this.storage.addEnergy(this.energyGeneration, false);
                    stateChanged = true;
                }
                else if (isRaining)
                {
                    this.energyGeneration = (int) (Integer.MAX_VALUE * 0.75);
                    if (this.storage.getEnergyStored() != this.storage.getMaxEnergyStored()) this.storage.addEnergy(this.energyGeneration, false);
                    stateChanged = true;
                }
                else
                {
                    this.energyGeneration = Integer.MAX_VALUE;
                    if (this.storage.getEnergyStored() != this.storage.getMaxEnergyStored()) this.storage.addEnergy(this.energyGeneration, false);
                    stateChanged = true;
                }
            }
