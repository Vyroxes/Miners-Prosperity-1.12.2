...
public class EmeraldBackpack extends Backpack
{
    public EmeraldBackpack(String name)
    {
        super(name);
        this.addPropertyOverride(new ResourceLocation("minersprosperity", "open_emerald_backpack"), ((stack, worldIn, entityIn) -> {
    ...
    public @NotNull ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn)
    {
        ...
        if (!worldIn.isRemote)
        {
            playerIn.openGui(MinersProsperity.INSTANCE, GuiHandler.GuiTypes.EMERALD_BACKPACK.ordinal(), worldIn, (int) playerIn.posX, (int) playerIn.posY, (int) playerIn.posZ);
    ...
    public static InventoryBackpack getBackpackInventory(ItemStack itemStack)
    {
        ...
        {
            InventoryBackpack inventory = new InventoryBackpack(54, "Iron Backpack");
            inventory.readFromNBT(compound.getCompoundTag("BackpackInventory"));
            return inventory;
        }
        else
        {
            return new InventoryBackpack(54, "Iron Backpack");
        }
    }
}